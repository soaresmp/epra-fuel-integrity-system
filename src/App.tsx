...
const FuelIntegrityApp = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([depotsData, stationsData, txnData, stockRes, incidentsData]) => {
        setDepots(depotsData);
        // Map flat DB fields to the nested inspection object the UI expects
        setGasStations(
          stationsData.map((s: any) => ({
            id: s.id,
            name: s.name,
            location: s.location,
            company: s.company,
            ...
            ...(s.inspectionLastDate
              ? { inspection: { lastDate: s.inspectionLastDate, result: s.inspectionResult, footage: s.inspectionFootage } }
              : {}),
          }))
        );
  ...
  const handleConfirmDeliveryFromDetail = (txnId: string) => {
    api.updateTransactionStatus(txnId, 'completed').catch(console.error);
  ...
  const handleConfirmTransitLoad = () => {
    if (transitLoadRegistration) {
      ...
      if (exists) {
        api.updateTransactionStatus(txn.id, 'in-transit').catch(console.error);
      } else {
              ...
              <div key={i} className={`bg-white rounded-lg shadow p-4 border-l-4 ${ep.priority === 1 ? 'border-red-500' : ep.priority === 2 ? 'border-orange-500' : ep.priority === 3 ? 'border-yellow-500' : 'border-green-500'}`}>
                ...
                <div className="border-t pt-2">
                  ...
                  <div className="space-y-1">
                    {ep.actions.map((action: any, j: number) => (
